public class NumberTheory {
    // calculates x S.T. x = a (mod m) and x = b (mod n)
    ...
    // Bonus Extended Euclidean algorithm, returns an array for [g, x, y] 
    ...
    // evaluates a^b (mod p)
    static long modRecPow(long a, long b, long p) {
        if (b == 0) {
            return 1;
        }
        long res;
        ...
        // (a * a)^(b/2) == a^b/2 * a ^ b/2
        return (res * modRecPow((a * a) % p, b >> 1, p)) % p;
    }
